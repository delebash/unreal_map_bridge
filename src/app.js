...
var mapSize = 17.28;
var tileSize = 1.92;
let outputSizePlus = 2017
let outputSize2 = 2017
...
function sanatizeMap(map, xOffset, yOffset) {
    const citiesmapSize = outputSizePlus;
...
function getHeightmap(mode = 0, callback) {autoSettings
    pbElement.value = 0;
    ...
    let extent = getExtent(grid.lng, grid.lat, mapSize / outputSize2 * outputSizePlus);
    ...
    let timer = window.setInterval(function () {
        ...
        if (isDownloadComplete(tiles, vTiles)) {
            ...
            switch (mode) {
                ...
                case 1:
                    citiesmap = toCitiesmap(sanatizedMap, watermap);
                    png = UPNG.encodeLL([citiesmap], outputSizePlus, outputSizePlus, 1, 0, 16);
...
function toHeightmap(tiles, distance) {
    ...
    // in heightmap, each pixel is treated as vertex data, and 1081px represents 1080 faces
    // therefore, "1px = 16m" when the map size is 17.28km
    let heightmap = Create2DArray(Math.ceil(outputSize2 * (distance / mapSize)), 0);
